...
- 包名：`com.mediavault`
- 当前版本：**0.3.13**（播放进度记忆；纵向滑动手势不抢进度；中心 overlay 无重复 Toast）

## 功能概要

- 底栏：主页 / 搜索 / 合集 / 刮削；顶栏：重读、数据、导入 library.json、设置
- **播放器**：沉浸底栏 1dp 细线；单击显控件；**仅横向**长按/滑动松手 seek；纵向滑动交给系统；倍速/快进快退/拖动仅屏幕中央提示（无 Toast）
- **进度记忆**：按 path 保存；手势 seek、进度条、±10s 按钮、每 10s、退出时写入；再次进入自动续播（近片尾清零）
- **搜索**：防抖、异步封面 LRU、标签单行
- **设置**：本地媒体根；WebDAV/FTP/SMB；WebDAV 测试连接
...
产出：`MediaVault_<versionName>_debug.apk`（如 `MediaVault_0.3.13_debug.apk`）。
