...
- **库维护优先**：新增库诊断、完整问题列表、筛选分页、源健康、远程能力、刮削证据、TMDB 复核队列、备份导出、诊断包和导入回滚，先解决重复、失效、未匹配、缺封面、远程不可解释失败、问题证据留存和迁移恢复等可维护性问题。
...
- **导入备份包**：先预检 ZIP 和来源版本，再覆盖媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好。导入前会在应用私有目录创建内部回滚快照；失败会自动回滚。备份中已脱敏的远程密码和 TMDB Key 会尽量保留当前配置，缺失的凭据需要导入后手动补充。
