...
# 生成 ../MediaVault_0.5.11_debug.apk
```
...
- **导入备份包**：先预检 ZIP、schema、内容清单、来源版本、媒体/目录/远程数量、刮削记录和偏好条目，再覆盖媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好。导入前会在应用私有目录创建内部回滚快照；失败会自动回滚。备份中已脱敏的远程密码和 TMDB Key 会尽量保留当前配置，缺失的远程凭据会在导入结果详情中列出，并可直接进入对应远程配置编辑；目录管理会高亮导入后仍需补密码的远程；导入后也可手动复检源健康。
