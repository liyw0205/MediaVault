...
# 生成 ../MediaVault_0.5.10_debug.apk
```
...
- **备份包**：媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好；默认脱敏远程密码和 TMDB Key，不包含封面缓存和远程播放缓存。
- **诊断包**：版本、设备、配置摘要、库诊断快照、源健康、远程能力和最近远程错误；生成时不主动触网，源健康使用最近一次“重新检查”的结果。
