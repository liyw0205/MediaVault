...
- **库维护优先**：新增库诊断、完整问题列表、筛选分页、源健康、远程能力、刮削证据、TMDB 复核队列、备份导出和诊断包，先解决重复、失效、未匹配、缺封面、远程不可解释失败和问题证据留存等可维护性问题。
...
| 数据管理 | 应用内查看媒体库、刮削记录、封面、远程 prefix/range 缓存占用并清理；导出备份包和诊断包 |
...
- **备份与诊断包** 在 **刮削 Tab -> 顶栏设置 -> 右侧侧栏 -> 设置** 中，生成 ZIP 后通过系统文件保存器写出。
- 没有底栏“设置”Tab。
- 连播设置在播放器 **列表 -> 连播** 中调整。
