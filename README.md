...
- 包名：`com.mediavault`
- 当前版本：**0.3.15**（远程 WebDAV/FTP/SMB 边下边播与前缀缓存；修复浏览直播 URI）
...
- **远程（0.3.15）**：库内 path 约定 `remote|<configId>|<posixPath>`；ExoPlayer 经 `mediavault-remote://` + `RemoteDataSource` 播放 **WebDAV / FTP / SMB**（HTTP Range、HEAD 长度、前缀流缓存）；设置里浏览远程目录可点视频直播；刮削页展示远程根并支持增量/重扫/移除库内条目
...
产出：`MediaVault_<versionName>_debug.apk`（如 `MediaVault_0.3.15_debug.apk`）。
