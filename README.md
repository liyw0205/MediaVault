...
- **导入备份包**：先预检 ZIP 和来源版本，再覆盖媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好。导入前会在应用私有目录创建内部回滚快照；失败会自动回滚。备份中已脱敏的远程密码和 TMDB Key 会尽量保留当前配置，缺失的凭据会在导入结果详情中提示补充。
- **导入回滚快照**：可在“备份与诊断包”里查看内部回滚快照数量、时间、大小，并按需清空；清空不会影响当前媒体库。
