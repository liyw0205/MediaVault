...
- **导入备份包**：先预检 ZIP、schema、内容清单和来源版本，再覆盖媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好。导入前会在应用私有目录创建内部回滚快照；失败会自动回滚。备份中已脱敏的远程密码和 TMDB Key 会尽量保留当前配置，缺失的远程凭据会在导入结果详情中列出，并可直接进入对应远程配置编辑；目录管理会高亮导入后仍需补密码的远程；导入后也可手动复检源健康。
- **导入回滚快照**：可在“备份与诊断包”里查看内部回滚快照数量、时间、大小，手动选择某个快照恢复；恢复前会预览快照 schema、将恢复的文件/偏好数量和缺失项；也可按需清空，清空不会影响当前媒体库。
