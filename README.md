...
| 编译目标 | Android SDK 34 |
| 当前版本 | **0.5.13**（versionCode 96） |

## 0.5.x 方向

0.5.x 的主线是把已经完整的媒体库能力做成可解释、可修复、可备份的可信媒体库，而不是继续扩张新业务面。
...
# 生成 ../MediaVault_0.5.13_debug.apk
```
...
- **备份包**：媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好；默认脱敏远程密码和 TMDB Key，不包含封面缓存和远程播放缓存。生成后会回读 ZIP，自检 manifest、schema/kind、必要条目、内容清单和字节数；空库也会写入可导入的空媒体库 JSON。
- **诊断包**：版本、设备、配置摘要、库诊断快照、源健康、远程能力和最近远程错误；生成时不主动触网，源健康使用最近一次“重新检查”的结果。生成后会回读 ZIP，自检 manifest、schema/kind、必要条目、内容清单和字节数。
- **导入备份包**：先预检 ZIP、schema、内容清单、来源版本、媒体/目录/远程数量、刮削记录和偏好条目；预检会校验 manifest 内容清单、必要条目、条目字节数、额外条目和可选 JSON/偏好文件，并显示“清单校验”和“可选 JSON”摘要。损坏的播放进度、观看历史、字幕偏好或播放 UI 偏好会在确认导入前被拦截；损坏的库诊断快照会被警告并在导入后重新生成。确认导入后会二次解析备份包，二次解析通过才创建内部回滚快照并覆盖媒体库、目录配置、远程配置、刮削记录、播放进度和基础偏好；失败会自动回滚。备份中已脱敏的远程密码和 TMDB Key 会尽量保留当前配置，缺失的远程凭据会在导入结果详情中列出，并可直接进入对应远程配置编辑；目录管理会高亮导入后仍需补密码的远程；导入后也可手动复检源健康。
